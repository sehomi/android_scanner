...
//                                dist(home_lat, home_lon, lat, lon);
                                binding.distState.setText(String.format("%.1f",dist(home_lat, home_lon, lat, lon)));
...
//                    Log.e(TAG, "onResume");
    ...
    private double dist(double lat1, double lon1, double lat2, double lon2){
        double xy1[] = utm_of_latlng(lat1, lon1);
        double xy2[] = utm_of_latlng(lat2, lon2);

        return Math.sqrt( (xy1[0] - xy2[0])*(xy1[0] - xy2[0]) + (xy1[1] - xy2[1])*(xy1[1] - xy2[1]));
    }

    private double[] utm_of_latlng(double Lat, double Lon)
    {
...
//                                Log.e(TAG, "---- ter 8");
                                ...
                                boolean info_window_shown = AllMarkers.get(idx).marker.isInfoWindowShown();
                                AllMarkers.get(idx).marker.remove();
                                AllMarkers.set(idx, mm);

                                if (info_window_shown)
                                {
                                    AllMarkers.get(idx).marker.showInfoWindow();
                                }
